 ...
 *
 * 历史：原静态图 npcs/xiaofang.png (64×80) → 升级为 8 帧待机动画
 *
 * 接口（与旧接口完全一致，调用方零修改）：
...
(function() {
  ...
  function _drawFallback(ctx, x, y) {
    ...
    ctx.beginPath();
    ctx.ellipse(x + 16, y + 30, 9, 15, 0, 0, Math.PI * 2);
    ctx.fill();
    // 头
  ...
  function drawXiaofang(ctx, x, y, dir, time) {
    if (window.NpcSpriteRenderer && window.NpcSpriteRenderer.isReady &&
      ...
      window.NpcSpriteRenderer.draw(ctx, 'xiaofang', x, y, time);
    } else {
