/**
 * NpcXiaofang — 小芳 NPC 美术贴图模块
 *
 * PHASE 18+：用美术成品图替代原程序化绘制（characters.js drawXiaofang）
 *   - 美术稿：assets/images/npcs/xiaofang.png（64×80 RGBA，符合 ART_SPEC v1.1 标准尺寸）
 *
 * 绘制约定（与 ART_SPEC v1.1 §1.2 一致，与其他 NPC 美术模块完全相同）：
 ...
 *
 * 兜底：图片未加载/失败时画极简素色椭圆（黄色裙装少女风），避免 NPC 完全消失影响交互。
 *
 * 接口：
 *   window.NpcXiaofang.draw(ctx, x, y, dir, time)
 *   签名与 drawXiaofang 完全一致，便于调用方零修改替换。
 */
(function() {
  'use strict';

  const SRC = 'assets/images/npcs/xiaofang.png';
  ...
  function _ensureLoaded() {
    ...
    img.onload = () => {
      _img = img;
      _loading = false;
      console.log('[npc-xiaofang] 美术图加载完成:', SRC, `${img.width}x${img.height}`);
    };
    img.onerror = () => {
      _failed = true;
      _loading = false;
      console.warn('[npc-xiaofang] 美术图加载失败，将持续走兜底椭圆:', SRC);
    };
    img.src = SRC;
  }
  ...
  function _drawFallback(ctx, x, y) {
    ...
    ctx.beginPath();
    ctx.ellipse(x + 16, y + 30, 11, 17, 0, 0, Math.PI * 2);
    ctx.fill();
    // 头（少女肤色）
    ctx.fillStyle = '#F5DCC0';
    ctx.beginPath();
    ctx.arc(x + 16, y + 9, 6, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  }
   ...
   */
  function drawXiaofangImage(ctx, x, y, dir, time) {
    _ensureLoaded();
