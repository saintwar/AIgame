...
## 7. 老板拍板记录（2026-06-03 全部按推荐定稿）

> 老板原话："全按推荐"——主策建议默认全部接受，主程按下表数值/策略硬编码实施。

| # | 决策点 | ✅ 老板拍板（最终） | 落地说明 |
|---|---|---|---|
| Q1 | **R_SCARE 惊吓半径** | **120px**（≈ wanderArea 半径中位） | §2.1 实施常量定 `R_SCARE = 120` |
| Q2 | **受惊期间二次抛漂** | **刷新 fleeTimer**（真实感 + 作妖惩罚） | `applyScare()` 命中已 flee 群直接重置 `fleeTimer = randInt(2,5)s`，不区分初次/二次 |
| Q3 | **受惊期 hover 浮窗 UI** | **不动**（保持解耦，浮窗只读语义不变） | 不补红色配色档，主美无追加任务 |
| Q4 | **黑漂概率** | **10%**（起步值，邀测看反馈再调；预留 5%~15% 调整带） | §3.4 概率表锁死 35/30/25/10，不开运行时配置 |
| Q5 | **试探/偷吃浮漂晃动振幅** | **±2px / ±3px**（像素风内眼可辨） | §3.5 偏移表锁死，主美不再评估 |
| Q6 | **浮窗 #FFD700 金边** | **保留**（属 UI 元素层，与"鱼群外框"语义解耦） | 删黄框只删 `_drawHighlight` (L196 鱼群外框)，不动 `_drawPopup` (L257) 浮窗描边 |
| Q7 | **D11 大鱼宿主群** `groupSlot` | **改为 'auto' 等概率随机选 1 群**（D14 随机化后无固定中央群） | D11 spec §3.2 同步修订（本次 push 一并改），新 roll 路径走"3 群等概率随机"；留存复活路径仍按 entry 中持久化的 groupSlot 复活 |

> 至此 D14 spec **全部决策点拍板**，无遗留 TBD，主程可直接落地。
