...
> **⚠️ 老板拍板 B（2026-06-02）：保留越档悬念，不做"力度=难度"确定映射。**
> 浮漂力度只是"第一印象预告"，**不锁定**博弈难度。抖得猛 → 大概率难（heavy 多对应稀有/强 behavior），但**保留小概率反差**（猛抖出软杂鱼 / 轻抖出硬骨头）。这与 D5 现有的 70/30 越档扰动哲学一脉相承——力度透传只调"起手张力第一印象"，真实难度仍由 behavior + HP 决定，**严禁**把 biteLevel 直接当难度系数硬乘进张力曲线。
...
5. 越档悬念保留（拍板 B）：猛抖**大概率**难、但小概率反差成立——猛抖出杂鱼=快速收尾 / 轻抖出大物=意外硬仗；**不得**出现"抖得猛就一定难"的确定映射 ✅
